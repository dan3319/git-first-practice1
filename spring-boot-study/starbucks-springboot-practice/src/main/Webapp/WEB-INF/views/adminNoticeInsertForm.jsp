...
<body>
    <div class="card">
        <div class="card-header1">
            <h1><a href="/adminNoticeList">다은의 공지사항 글 등록</a></h1>
        </div>
        <form action="/adminNoticeInsert" method="post" id="form1" onSubmit="return false" enctype="multipart/form-data">
	        <div class="card-write">
	            <div class="myinfo">
	                작성자<input type="text" id="writer" name="writer" placeholder="이름을 입력하세요.">
	            </div>
	            <div class="title-w">
    ...
    <script>
    	function prevCheckTextBox() {
    		if (!$('#writer').val()) {	// 이름 관련 dom
    			alert('이름을 입력하세요.');	// 이름 입력하라고 팝업 뜸.
    			$('#writer').focus();		// 이름 입력 칸으로 포커스 이동
    			
    			return;
    		}
    		if ($('#writer').val().length > 5) { // 이름 관련 dom
